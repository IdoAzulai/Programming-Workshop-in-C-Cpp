...
#define FIRST_APPEAR 1
#define MAX_TWEET_LEN 2000
/**
 * insert MarkovChain struct
 */
typedef struct MarkovChain {
  LinkedList *database;// pntr to nodelst, holding all unique words as nodes
} MarkovChain;

typedef struct MarkovNode {
  char *data; // just word1
  struct MarkovNodeFrequency **frequencies_list; // holds all words2
  int freq_array_size; // holds size of the whole freq array
} MarkovNode;

typedef struct MarkovNodeFrequency {
  MarkovNode *markov_node; // pointer to word2 (that comes after word1)
  int frequency; // freq of word2 coming after word1
} MarkovNodeFrequency;
